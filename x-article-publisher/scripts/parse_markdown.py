...
def split_into_blocks(markdown: str) -> list[str]:
    ...
    for line in lines:
        ...
        # Handle code block boundaries - support both ``` and ```` (nested)
        ...
        if fence_match:
            ...
            if in_code_block:
                # Check if this fence closes the current block (same or more backticks)
                if len(fence) >= len(code_block_fence):
                    # End of code block
                    in_code_block = False
                    if code_block_lines:
...
def extract_images_with_block_index(markdown: str, base_path: Path) -> tuple[list[dict], str, int]:
    ...
    for i, block in enumerate(blocks):
        ...
        if match:
            ...
            # Resolve relative paths, but keep HTTP URLs as-is
...
def extract_title(markdown: str) -> tuple[str, str]:
    ...
    for idx, line in enumerate(lines):
        ...
        if stripped.startswith('## '):
            ...
            break
        # H3 - use as title but don't remove
        if stripped.startswith('### '):
            title = stripped[4:].strip()
            break
        # First non-empty, non-special line
        title = stripped[:100]
        break
...
def preprocess_text(text: str) -> str:
    """Preprocess text for X Articles compatibility.
    
    Handles:
    - Arrow symbols (→) - keep as-is, X supports Unicode
    - Multiple spaces for alignment - convert to proper formatting
    - Special characters that might cause issues
    """
    # Replace multiple spaces (alignment) with single space
    ...
    for line in lines:
        ...
        content = line.lstrip()
        # Replace multiple consecutive spaces in content with single space
        content = re.sub(r'  +', ' ', content)
        processed_lines.append(' ' * min(leading_spaces, 4) + content)  # Max 4 spaces indent
...
def markdown_to_html(markdown: str) -> str:
    ...
    def convert_code_block(match):
        ...
        for line in lines:
            ...
            line = line.replace('<', '&lt;')
            line = line.replace('>', '&gt;')
            # Preserve spaces in code blocks using &nbsp; for leading spaces
            if line.startswith(' '):
                leading = len(line) - len(line.lstrip())
                line = '&nbsp;' * leading + line.lstrip()
            escaped_lines.append(line)
        
        # Join lines with <br> for display
