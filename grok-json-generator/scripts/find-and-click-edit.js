  ...
  try {
    ...
    const targetElement = await grokPage.evaluate(() => {
      ...
      allElements.forEach(el => {
        ...
        // 必须包含目标文本和链接
        if (!innerText.includes(targetText) || !innerText.includes(link1)) {
          return;
        }
    ...
    // 点击第一个可见的按钮
    console.log(`\n🖱️ 点击第 [${visibleButtons[0]}] 个按钮...`);

    const allButtons = await targetElement.$$('button');
    await allButtons[visibleButtons[0]].click();
